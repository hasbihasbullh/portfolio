  ...
  useEffect(() => {
    if (listRef.current) {
      const selectedElement = listRef.current.querySelector(
        `[data-command-item="${selectedIndex}"]`
      ) as HTMLElement;
      if (selectedElement) {
        ...
        <Dialog.Portal>
          ...
          >
            ...
            <div className="flex items-center gap-3 px-4 border-b border-zinc-800/80">
              ...
              />
              <Dialog.Close className="bg-zinc-900 border border-zinc-800 hover:bg-zinc-800 text-zinc-400 hover:text-zinc-200 px-2 py-1 rounded text-xs transition-colors">
                Esc
              </Dialog.Close>
            </div>
                ...
                <div ref={listRef} className="space-y-4">
                  {Object.entries(groupedItems).map(([category, catItems]) => (
                    <div key={category} className="space-y-1">
                      <h3 className="px-3 pt-2 pb-1 text-[10px] font-semibold text-zinc-500 uppercase tracking-wider">
                        {category}
                      </h3>
                      {catItems.map((item) => {
                          ...
                          >
                            <div className="flex items-center gap-3 min-w-0">
                              ...
                              <div className="min-w-0">
                                ...
                                {item.subtitle && (
                                  <p className="text-[10px] text-zinc-500 truncate max-w-sm sm:max-w-md">
                                    {item.subtitle}
                                  </p>
                                )}
                              </div>
                            ...
                            {isSelected && (
                              <span className="text-[10px] text-zinc-500 bg-zinc-900 border border-zinc-700/50 px-1.5 py-0.5 rounded font-mono">
                                Enter
                              </span>
                            )}
                          </div>
            ...
            <div className="flex items-center justify-between px-4 py-2.5 bg-zinc-900/40 border-t border-zinc-800/60 text-[10px] text-zinc-500">
              ...
              <div>
                <span>Press <kbd className="bg-zinc-900 px-1 py-0.5 rounded border border-zinc-700/60 text-[9px] font-mono">Esc</kbd> to close</span>
              </div>
            </div>
          </Dialog.Content>
        </Dialog.Portal>
